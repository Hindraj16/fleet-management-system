...
export default function HistoryPlayback() {
    ...
    async function loadVehicles() {
      try {
        ...
        if (!ignore && Array.isArray(data) && data.length > 0) {
          setVehicles(data);
          // Set initial active vehicle ID without setting searchTerm text
  ...
  // Filter local database vehicles by input text
  ...
  const handleSelectVehicle = (v) => {
    setSelectedVehicle(v.id.toString());
  ...
  return (
    <div className="w-full h-screen overflow-hidden bg-gray-50 flex flex-col font-sans text-gray-800 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
      <main className="flex-1 w-full px-8 py-6 flex flex-col space-y-6 overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
        <h1 className="text-3xl font-bold text-gray-800">
          Vehicle Route History Playback
        </h1>

        {/* Control Panel */}
        <div className="w-full bg-white p-5 rounded-xl shadow-md border border-gray-100 flex flex-wrap gap-4 items-end">
        ...
        {error && (
          <div className="w-full bg-red-50 text-red-600 p-4 rounded-lg border border-red-200 text-sm">
            {error}
          </div>
        )}
        ...
        {history.length > 0 && (
          <>
            ...
            {/* Map Section */}
            <div className="w-full bg-white p-4 rounded-xl shadow-md border border-gray-100 flex-1 flex flex-col min-h-[500px]">
              <div className="h-full w-full rounded-lg overflow-hidden flex-1 min-h-[450px]">
                <MapContainer
